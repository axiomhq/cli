...
func NewIngestCmd(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "ingest (-d|--dataset) <dataset-name> [--timestamp-field <timestamp-field>] [--timestamp-format <timestamp-format>]",
		Short: "Ingest data",
		...
		Example: heredoc.Doc(`
			# Pipe the content of a JSON log file into a dataset named "logs":
			$ echo log.json | axiom ingest -d logs
		`),
