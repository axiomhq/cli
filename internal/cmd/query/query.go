...
func NewCmd(f *cmdutil.Factory) *cobra.Command {
	...
	cmd.Flags().StringVar(&opts.StartTime, "start-time", "", "Start time of the query - may also be now or a relative time eg: -2w, -7d, -24h, -20m")
	cmd.Flags().StringVar(&opts.EndTime, "end-time", "", "End time of the query - may also be now or a relative time eg: -2w, -7d, -24h, -20m")
...
func timeStrToTime(timeStr string, timestampFormat string, now time.Time) (time.Time, error) {
	if timestampFormat != "" {
		// parse the timestamp as absolute because we have a definitive format
		return time.Parse(timestampFormat, timeStr)
	}

	// try "now" and relative dates first, expanding day/week suffixes
	if t, ok := cmdutil.RelativeTime(timeStr, now); ok {
		return t, nil
	}
...
func complete(opts *options) (err error) {
	...
	if ts := opts.StartTime; ts != "" {
		opts.startTime, err = timeStrToTime(ts, opts.TimestampFormat, now)
		if err != nil {
			return err
		}
	}
	...
	if ts := opts.EndTime; ts != "" {
		opts.endTime, err = timeStrToTime(ts, opts.TimestampFormat, now)
		if err != nil {
			return err
		}
	}
