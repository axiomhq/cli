...
func runLicense(ctx context.Context, opts *licenseOptions) error {
	...
	if opts.IO.IsStdoutTTY() {
		header = func(_ io.Writer, trb iofmt.TableRowBuilder) {
			...
			trb.AddField("Monthly ingest (GB)", cs.Bold)
			trb.AddField("Max users", cs.Bold)
			trb.AddField("Max teams", cs.Bold)
			trb.AddField("Max datasets", cs.Bold)
	...
	contentRow := func(trb iofmt.TableRowBuilder, _ int) {
		...
		trb.AddField(strconv.FormatUint(license.MonthlyIngestGB, 10), nil)
		trb.AddField(strconv.FormatUint(license.MaxUsers, 10), nil)
		trb.AddField(strconv.FormatUint(license.MaxTeams, 10), nil)
		trb.AddField(strconv.FormatUint(license.MaxDatasets, 10), nil)
